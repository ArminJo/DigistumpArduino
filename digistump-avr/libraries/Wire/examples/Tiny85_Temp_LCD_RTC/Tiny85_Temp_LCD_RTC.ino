...
void Init_Temp(){ // Setup the DS1621 for one-shot mode
  ...
  Wire.send(0xAC);                 // Access Command Register
  Wire.send(B00000001);            // Using one-shot mode for battery savings
  //Wire.send(B00000000);          // if setting continious mode for fast reads
...
void Get_Temp(){  // Get the temperature from a DS1621
  Wire.beginTransmission(DS1621_ADDR);
  Wire.send(0xEE);                 // if one-shot, start conversions now
  Wire.endTransmission();          // Send 1 byte to the slave
  delay(750);                           // if one-shot, must wait ~750 ms for conversion
  Wire.beginTransmission(DS1621_ADDR);
  Wire.send(0xAA);                 // read temperature (for either mode)
  Wire.endTransmission();          // Send 1 byte to the slave
  Wire.requestFrom(DS1621_ADDR,1); // Request 1 byte from slave
  tempC = Wire.receive();          // get the temperature
...
void Get_Time(){         // get the time and date from the DS1307 chip
  ...
  wireRet = Wire.requestFrom(DS1307_ADDR, 7);    // request 7 bytes from DS1307
  ...
  lcd.print(Wire.available(),DEC);     // testing Wire.available()
  delay(1500);
#endif    
  seconds = bcdToDec(Wire.receive());  // handle the 7 bytes received
  minutes = bcdToDec(Wire.receive());      
  hours = bcdToDec(Wire.receive());        
  day_of_week = Wire.receive();    
  days = bcdToDec(Wire.receive());          
  months = bcdToDec(Wire.receive());      
  years = bcdToDec(Wire.receive());  
#ifdef DEBUG
  lcd.clear();
